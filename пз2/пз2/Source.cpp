...
void byRandom(int numbersLine, int numbersColumn, int** matrix)
{
	...
	for (int counterLine = 0; counterLine < numbersLine; counterLine++)
	{
		for (int counterColumn = 0; counterColumn < numbersColumn; counterColumn++)
		{
			int znak = rand() % 3;//отрицательных чисел меньше
			int sign;
			if (znak == 1)
				sign = -1;
			else
...
void medianMatrixAndMinMax()
{
	...
	cout << "Min number = " << array[0] << "\n";
	cout << "Max number = " << array[numberLine * numberColumn - 1] << "\n";
...
int main()
{
	...
	cout << "Так как я узнал что можно объединять задания, то я объединил 2 и 4 задание в одно задание под №2) \n";
	int toggle;
	while (true) {
		do {
			cout <<"Press key to continue(1-4)\n" << "-1- Print matrix\n" << "-2- All tasks with arrays\n"
